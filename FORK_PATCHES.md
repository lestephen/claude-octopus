...
This fork carries 59 commits on top of `upstream/main` (currently at
...
| 56 | `696a145` | feat  | Render_script leg pipeline integration (closes GH #27). Petrics PR-25 empirically mapped 3 of 11 divergences as requiring this leg (stage size, glow ellipses tracking pet scale, sprite-vs-stage size mismatch) — petrics walked back their earlier "polish / deferred" framing of `render_script` in the .33 dogfood report. Now `render_diff.sh` is invoked PRE-DISPATCH (when `visual.render_script` is set) and the resulting screenshot is appended to `OCTO_AGENT_IMAGES` as a SECOND image alongside the mockup. Reviewers see both side-by-side without per-turn process orchestration. Q3 (proportion) preamble block rewritten from "eyeball the proportion" to concrete two-coord bounding-box methodology with petrics' worked example (pet sprite 30% mockup vs 50% rendered → 20pp delta → flag) AND adds detection for dependent/relative sizing (glow ellipses that should track parent scale). Two distinct finding categories surfaced: `visual-proportion-divergence` (named region's proportion differs >20%) and `rendered-divergence` (rendered pixel sample differs from mockup at same coord by deltaE > threshold — catches Tailwind/GL/alpha pipeline issues that token-only inspection misses). Render-failure path explicit: degraded Q3 block tells reviewer the leg failed at dispatch time and falls back to mockup-only. | Plausible — closes empirically-validated coverage gap |
| 57 | `d655827` | fix   | Self-dogfood-driven hardening of .47/.48 via 4 rounds of multi-LLM consensus. Both providers returned SEV-1 BLOCKING in round 1 on the shipped .47/.48 code, surfacing two REAL bugs: (1) `STATED INTENT` block fenced with triple-backticks — commit messages contain ``` frequently, fence closure → prompt-injection vector; (2) `rendered-divergence` Q3 asked reviewer to invoke `sample_pixel.py` twice mid-review — the exact mid-review-tool-orchestration failure mode .48's pre-dispatch render attach was designed to remove. Round-1 codex SEV-1 about severity schema was a hallucination (actual schema in `agent_prompt_base` is `normal\|nit\|pre-existing`, not the `critical/high/medium/low` codex claimed). Round-4 (user-approved override of 3-round cap) lands the final fix set: (a) switched fenced block to XML-tagged `<stated_intent>` AND applied proper XML entity escaping (`&` → `&amp;`, `<` → `&lt;`, `>` → `&gt;`) — neutralizes literal `</stated_intent>`, whitespace-tolerant `</stated_intent >`, mixed-case `</Stated_Intent>`, and nested ampersand vectors empirically; (b) removed contradictory `bash render_diff.sh` instruction from TOOLS YOU SHOULD CALL, replaced with NOTE explaining render ran pre-dispatch; (c) rewrote Q3 rendered-divergence methodology to rely on vision LLM's native image comparison instead of orchestrating tools mid-review; (d) scoped "Do NOT run pixel-sampling tools" to "FOR THIS RENDERED-vs-MOCKUP COMPARISON" so code-vs-mockup sampling protocol is preserved; (e) `else` Q3 branch (no render_script set) now explicitly forbids both `rendered-divergence` and `visual-proportion-divergence`; (f) replaced `<x> <y>` and `<hex1> <hex2>` placeholders in tool examples with `X Y` and `HEX1 HEX2` (shell-redirection bug). Round 4 NO BLOCKING from both providers. | Plausible — defensive hardening of recent change |
| 58 | `6884775` | feat  | Gemini auth detection: centralized resolver + upstream-#18927 stale-blob handling (closes GH #28). Mac dogfood found `~/.gemini/.env` could contain a JSON OAuthCredentials wrapper masquerading as `GEMINI_API_KEY=` (per upstream gemini-cli#18927 — Keychain access fails on Homebrew installs / some Linux envs, fallback serializes credentials into .env as JSON). Headless gemini reads the JSON string as the literal API key → 400 INVALID_ARGUMENT. NEW: `octo_resolve_gemini_auth` canonical resolver in `scripts/lib/preflight.sh` returns one of `api-key\|stale-blob\|oauth\|keychain\|none` with strict precedence (env-var → stale-blob → oauth → keychain → none; stale-blob MUST precede oauth because gemini-cli's .env loader shadows oauth_creds). Two-stage shape validation rejects JSON OAuthCredentials wrappers in env-vars AND in ~/.gemini/.env (must start with `{` AND contain `accessToken\|tokenType\|expiresAt\|serverName` to avoid false-positive on legitimate `{`-prefixed enterprise tokens). New `octo_gemini_dispatch_allowed` allowlist — `api-key/oauth/keychain` allowed, `stale-blob/none` denied. New `octo_warn_gemini_stale_blob` TTY-gated + sentinel-guarded remediation message. 11 sites refactored to call the resolver: `cmd_detect_providers` (live-echo + cache-write), `preflight_check`, `providers.sh::detect_providers`, `providers.sh::check_provider_health`, `model-resolver.sh::is_agent_available_v2`, `orchestrate.sh::is_agent_available` (with inline shape-validating fallback for when preflight.sh isn't sourced), `orchestrate.sh::has_gemini`, `orchestrate.sh::init-workflow::_gemini_ok`, `config-display.sh::save_user_config`, `config-display.sh` display block, `setup.md` STEP 1 initial scan (sources preflight.sh from `${OCTOPUS_PLUGIN_DIR:-~/.claude-octopus/plugin}` if available, two-stage inline fallback otherwise). 13 rounds of consensus. Final NO SEV-1 from both providers; documented residual SEV-2s (yaml-workflow + provider-routing + smoke/doctor/interactive/auth not yet refactored) filed as GH #29 (provider factory pattern). Plus `setup.md` `Platform-specific gotchas` section documents codex stdin-positional-arg hang + gemini Keychain-broken-on-Homebrew caveat per user's Mac+Windows dogfood. | Plausible — closes a real Mac dogfood failure with single-source-of-truth resolver |
| 59 | _pending_ | fix   | Two cross-platform / namespace bug fixes bundled. **(a) `/octo:resume` → `/octo:agent-resume` rename** — user reported that typing `/resume` inside a Claude Code session landed in octo's agent-resume flow instead of the built-in session picker. Root cause: octo's command frontmatter `command: resume` registered `/octo:resume` AND collided semantically with Claude Code's built-in `/resume`. Fix: rename to `command: agent-resume` (matches the orchestrator's actual `agent-resume` subcommand), add `resume → agent-resume` alias in `hooks/user-prompt-submit.sh::octo_alias_for` for back-compat. Also renamed `.cursor-plugin/commands/octo-resume.md` → `octo-agent-resume.md` for the cursor variant. Updated `docs/COMMAND-REFERENCE.md`. **(b) Windows Git Bash `$HOME` vs `$USERPROFILE` divergence breaks codex + gemini auth detection. User reported: on Windows running Git Bash inside Claude Code, `$HOME=/z/` (mapped network drive) while actual codex/gemini auth files live at `$USERPROFILE=/c/Users/<name>/.codex/auth.json` etc. The `[[ -f "$HOME/.codex/auth.json" ]]` checks at `preflight.sh:638` and 8+ other sites all returned false → codex+gemini silently skipped at dispatch despite being fully authenticated. Fix: NEW helpers `octo_user_file_exists` and `octo_user_file_path` in `preflight.sh` check both `$HOME/<rel>` and `$USERPROFILE/<rel>` (USERPROFILE only when set AND distinct from HOME — no-op on Linux/Mac). Applied helper or inline `\|\|` fallback to all codex+gemini auth-detection sites in the hot dispatch path: `preflight.sh` (3 sites: resolve_gemini_auth, env_corruption, preflight_check, plus cmd_detect_providers's 3 codex auth.json checks), `orchestrate.sh::has_openai`, `providers.sh::detect_providers` codex section (with source-on-demand for the helper), `config-display.sh` (7 sites: STEP 5 codex tier, STEP 6 gemini autodetect, all_good checks, doctor display, check_first_run, check_codex_auth_freshness), `doctor.sh::doctor_check_auth` (codex+gemini+any_auth gate), `auth.sh::check_codex_auth` + status handler, `setup.md` STEP 1 initial scan. Smoke-tested all 6 divergence scenarios (HOME-only, USERPROFILE-only, both, neither, USERPROFILE unset, USERPROFILE==HOME). Consensus check: codex SEV-2 ONLY (4 deferred sites + stale-blob-across-HOMEs edge case), gemini NO BLOCKING; minority SEV-2 allows ship per gate. Remaining sites (qwen, copilot, cursor-agent, model-resolver branches, smoke.sh, interactive.sh, plus the stale-blob divergence edge case) filed as GH #30. | **Yes — clear cross-platform bug; the underlying pattern affects any plugin reading `$HOME/.<provider>/` on Windows Git Bash** |
